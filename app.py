...
def create_donut_chart(values, labels, title):
    if not any(values):
        ...
        fig.update_layout(
            ...
            annotations=[{"text": "N/A", "xref": "paper", "yref": "paper", "showarrow": False, "font": {"size": 28}}]
        )
        return fig
    fig = go.Figure(data=[go.Pie(
        labels=labels, values=values, hole=.5, textinfo='label+percent',
        insidetextorientation='radial', marker_colors=['#3b71ca', '#adb5bd']
    )])
    fig.update_layout(
...
@app.callback(
    ...
    [Input(f"btn-nav-{view}", "n_clicks") for view in ['generador', 'configuracion', 'registros', 'historicos', 'graficos']],
)
...
@app.callback(
    [Output(f"btn-nav-{view}", "className") for view in ['generador', 'configuracion', 'registros', 'historicos', 'graficos']],
    [Input(f"btn-nav-{view}", "n_clicks") for view in ['generador', 'configuracion', 'registros', 'historicos', 'graficos']],
)
def update_nav_buttons_style(*args):
    triggered_id = ctx.triggered_id or "btn-nav-generador"
    base_class = "nav-button"
    styles = {view: base_class for view in ['generador', 'configuracion', 'registros', 'historicos', 'graficos']}
    active_view = triggered_id.split('-')[-1]
...
@app.callback(Output("notification-container", "children", allow_duplicate=True), Input("btn-pregen-omega", "n_clicks"), prevent_initial_call=True)
def handle_pregenerate_omega(n_clicks):
    ...
    return dbc.Alert(f"{message} (Tiempo: {time.time() - start:.2f}s)", color="success" if success else "danger", duration=10000)
...
def handle_generate_omega(n_clicks, *num_inputs):
    ...
    if all(num is None or num == '' for num in num_inputs):
        ...
        return [no_update] * 6 + [dbc.Alert("Error al generar.", color="warning", duration=5000), None]
    else:
        ...
        if eval_result.get("esOmega"): return [no_update] * 6 + [dbc.Alert("¡Tu combinación ya es Omega!", color="success", duration=6000), user_combo]
        adjusted, matches = adjust_to_omega(user_combo)
        if adjusted: return adjusted + [dbc.Alert(f"¡Ajuste exitoso! Se mantuvieron {matches} números.", color="info", duration=8000), adjusted]
        return [no_update] * 6 + [dbc.Alert("No se encontró un ajuste cercano.", color="danger", duration=8000), None]

@app.callback(Output("notification-container", "children", allow_duplicate=True), Output('store-validated-omega', 'data', allow_duplicate=True), Input("btn-analizar", "n_clicks"), [State(f"num-input-{i}", "value") for i in range(6)], prevent_initial_call=True)
def handle_analizar_combinacion(n_clicks, *num_inputs):
    ...
    if not isinstance(result, dict) or result.get("error"): return dbc.Alert(result.get("error", "Error en evaluación."), color="danger"), None
    ...
    body = [html.H4(title, className="alert-heading"), html.P(f"Tu combinación: {result.get('combinacion', [])}"), html.Hr(), html.Ul([html.Li(f"Pares: {pares.get('score')} / {pares.get('umbral')} {'✅' if pares.get('cumple') else '❌'}"), html.Li(f"Tercias: {tercias.get('score')} / {tercias.get('umbral')} {'✅' if tercias.get('cumple') else '❌'}"), html.Li(f"Cuartetos: {cuartetos.get('score')} / {cuartetos.get('umbral')} {'✅' if cuartetos.get('cumple') else '❌'}")])]
    return dbc.Alert(body, color=color, duration=10000), (combo if es_omega else None)

@app.callback([Output(f"num-input-{i}", "value", allow_duplicate=True) for i in range(6)] + [Output('store-validated-omega', 'data', allow_duplicate=True)], Input("btn-clear-inputs", "n_clicks"), prevent_initial_call=True)
...
def handle_register_omega(n_clicks, validated_omega, nombre, movil):
    ...
    if not validated_omega or not nombre or not movil: return dbc.Alert("Todos los campos son obligatorios.", color="warning", duration=4000)
...
def update_all_graphs(n_clicks):
    ...
    # --- CORRECCIÓN FINAL CON NUMPY ---
    # Añadimos np.integer a la comprobación de tipos
    if not isinstance(total_omega_class, (int, float, np.integer)):
        logger.error(f"Tipo inesperado para conteo Omega: {type(total_omega_class)}")
        return empty_fig, empty_fig, empty_fig, dbc.Alert("Error de tipo en los datos.", color="danger")
    # --- FIN DE LA CORRECCIÓN ---
    
    total_omega_class_int = int(total_omega_class)
    
    if total_omega_class_int == 0:
        msg = dbc.Alert("La Clase Omega no ha sido pre-generada.", color="warning", duration=5000)
        return empty_fig, empty_fig, empty_fig, msg
    
    total_posibles = C(39, 6)
    fig_universo = create_donut_chart([total_omega_class_int, total_posibles - total_omega_class_int], ['Clase Omega', 'Otras'], 'Universo de Combinaciones')
