...
VIEWS = ["generador", "graficos", "historicos"] + \
        (["monitoreo"] if config.DEBUG_MODE else []) + \
        ["omega-cero", "fenix", "registros", "configuracion"]
...
if __name__ == "__main__":
    ...
    # --- CALLBACKS DE LA PESTAÑA FÉNIX ---
    @app.callback(
        Output("graph-fenix-histogram", "figure"),
        Output("graph-fenix-boxplot", "figure"),
        Output("fenix-stats-summary", "children"),
        Input("btn-run-fenix-analysis", "n_clicks"),
        State('store-active-game', 'data'),
        prevent_initial_call=True
    )
    def update_fenix_analysis_graphs(n_clicks, game_id):
        if not fue_un_clic_real('btn-run-fenix-analysis'):
            return no_update, no_update, no_update

        # Ejecutar el script de análisis
        logger.info(f"Ejecutando script 'analyze_fenix_distribution.py' para el juego '{game_id}'...")
        try:
            subprocess.run([sys.executable, "analyze_fenix_distribution.py", game_id], check=True, capture_output=True, text=True)
            logger.info("Script de análisis Fénix completado con éxito.")
        except subprocess.CalledProcessError as e:
            logger.error(f"Falló la ejecución del script de análisis Fénix. Salida: {e.stdout}. Error: {e.stderr}")
            return go.Figure(), go.Figure(), dbc.Alert("Error ejecutando el script de análisis. Revise los logs del servidor.", color="danger")


        # Cargar los resultados
        analysis_path = os.path.join(config.DATA_DIR, f"{game_id}_fenix_analysis.json")
        try:
            with open(analysis_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
            return go.Figure(), go.Figure(), dbc.Alert("No se encontró el archivo de análisis. Asegúrese de que el script de cálculo se ejecutó correctamente.", color="warning")
        
        # Crear DataFrame para visualización
        df_ganadores = pd.DataFrame({'score': data['ganadores_scores'], 'grupo': 'Ganadores Históricos'})
        df_virgenes = pd.DataFrame({'score': data['virgenes_scores'], 'grupo': 'Candidatas Vírgenes'})
        df_plot = pd.concat([df_ganadores, df_virgenes], ignore_index=True)

        # Histograma
        fig_hist = px.histogram(df_plot, x="score", color="grupo", barmode="overlay",
                                title="Distribución de Scores Fénix",
                                labels={"score": "Score Fénix (Reactividad)", "grupo": "Grupo"})
        fig_hist.update_traces(opacity=0.75)

        # Box Plot
        fig_box = px.box(df_plot, x="grupo", y="score", color="grupo",
                         title="Comparativa de Reactividad",
                         labels={"score": "Score Fénix", "grupo": "Grupo"})
        
        # Resumen estadístico
        stats = data['stats']
        summary_table = dbc.Table([
            html.Thead(html.Tr([html.Th("Métrica"), html.Th("Ganadores"), html.Th("Vírgenes")])),
            html.Tbody([
                html.Tr([html.Td("Media"), html.Td(f"{stats['ganadores_mean']:.4f}"), html.Td(f"{stats['virgenes_mean']:.4f}")]),
                html.Tr([html.Td("Mediana"), html.Td(f"{stats['ganadores_median']:.4f}"), html.Td(f"{stats['virgenes_median']:.4f}")]),
                html.Tr([html.Td("Desv. Estándar"), html.Td(f"{stats['ganadores_std']:.4f}"), html.Td(f"{stats['virgenes_std']:.4f}")]),
                html.Tr([html.Td("# Muestras"), html.Td(f"{len(data['ganadores_scores']):,d}"), html.Td(f"{len(data['virgenes_scores']):,d}")]),
            ])
        ], bordered=True, striped=True, hover=True, className="mt-4")

        return fig_hist, fig_box, summary_table


    @app.callback(
        Output("view-content", "children"),
        [Input(f"btn-nav-{view}", "n_clicks") for view in VIEWS] # Usando la nueva lista
    )
    def render_view_content(*args):
        from modules import presentation
        triggered_id = ctx.triggered_id or "btn-nav-generador"
        
        # Mapa de vistas completo, incluyendo Fénix
        view_map = {
            "btn-nav-configuracion": presentation.create_configuracion_view,
            "btn-nav-registros": presentation.create_registros_view,
            "btn-nav-fenix": presentation.create_fenix_view, # Añadido
    ...
    @app.callback(
        [Output(f"btn-nav-{view}", "className") for view in VIEWS], # Usando la nueva lista
        [Input(f"btn-nav-{view}", "n_clicks") for view in VIEWS]  # Usando la nueva lista
    )
    def update_nav_buttons_style(*args):
        triggered_id = ctx.triggered_id or "btn-nav-generador"
        styles = {view: "nav-button" for view in VIEWS} # Usando la nueva lista
    ...
    def open_and_populate_deconstructor_modal(active_cell, table_data, game_id):
        if (
            not active_cell
            or active_cell.get("column_id") != "analizar"
            or not table_data
        ):
            return no_update, no_update
        ...
        try:
            ...
            logger.error(
                f"Error al extraer datos de la fila para el deconstructor: {e}"
            )
            return no_update, no_update
    ...
    def update_all_graphs(n_clicks, game_id):
        ...
        df_golden_trajectory = database.read_golden_trajectory(
            game_config["paths"]["db"]
        )  # LEER LÍNEA DORADA
        ...
        if not df_trajectory.empty:
            fig_trajectory.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="Score Original (Ganador)",
                    line=dict(
                        color="rgba(231, 76, 60, 0.9)", width=2
                    ),  # Rojo más visible
                )
            )
            fig_trajectory.add_trace(
                go.Scatter(
                    x=df_trajectory["concurso"],
                    y=df_trajectory["random_omega_score"],
                    mode="lines",
                    name="Score Original (Aleatorio)",
                    line=dict(
                        color="rgba(173, 216, 230, 0.7)", width=1.5
                    ),  # Azul claro para ruido
                )
            )

            # AÑADIR LA LÍNEA DORADA
            if not df_golden_trajectory.empty:
                fig_trajectory.add_trace(
                    go.Scatter(
                        x=df_golden_trajectory["concurso"],
                        y=df_golden_trajectory["elite_score_original_mean"],
                        mode="lines",
                        name="Score Original (Élite Reactiva)",
                        line=dict(
                            color="rgba(255, 215, 0, 0.9)", width=2, dash="dash"
            ...
            fig_trajectory.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="Score Actual (Ganador)",
                    line=dict(
                        color="rgba(52, 152, 219, 0.5)", width=1
                    ),  # Azul atenuado
                )
            )

            fig_trajectory.update_layout(
                title_text=f"Prueba de Fuego: Comparativa de Reactividad<br>({game_config['display_name']})",
                title_x=0.5,
                ...
                shapes=[
                    dict(
                        ...
                        y0=0,
                        y1=0,
                        x0=df_trajectory["concurso"].min(),
                        x1=df_trajectory["concurso"].max(),
    ...
    @app.callback(
        Output("kpi-banda", "children"),
        Output("kpi-ciclo", "children"),
        Output("kpi-estabilidad", "children"),
        Output("kpi-candidatas", "children"),
        Output("table-omega-cero-candidatas", "data"),
        Output("table-omega-cero-candidatas", "columns"),
    ...
    def update_omega_cero_dashboard(n_clicks, game_id):
        ...
        if not metrics:
            alert = dbc.Alert(
                "Datos de métricas no encontrados. Ejecute el script 'generate_omega_score_trajectory.py' primero.",
                color="warning",
            )
        ...
        columns = [
            ...
            for i in df_candidatas.columns
        ]
        ...
        if not df_candidatas.empty:
            fig = px.histogram(
                df_candidatas,
                x="simulated_original_score",
                nbins=50,
                title="Distribución de Scores Simulados (Candidatas)",
            )
            fig.add_vline(
                x=metrics["banda_normal_inferior"],
                line_width=2,
                line_dash="dash",
                line_color="green",
            )
            fig.add_vline(
                x=metrics["banda_normal_superior"],
                line_width=2,
                line_dash="dash",
                line_color="green",
            )
            fig.update_layout(
                template="simple_white", xaxis_title="Score Original Simulado"
            )
        else:
    ...
    def update_monitoring_graphs(n_clicks, game_id):
            ...
            for i, level in enumerate(["cuartetos", "tercias", "pares"]):
                ...
                fig_freq_dist.add_trace(
                    go.Scatter(
                        ...
                        y=df_freq_dist[f"freq_{level}_max"],
                        mode="lines",
                        line=dict(width=0),
                        showlegend=False,
                    ),
                ...
                fig_freq_dist.add_trace(
                    go.Scatter(
                        x=df_freq_dist["ultimo_concurso_usado"],
                        y=df_freq_dist[f"freq_{level}_min"],
                        mode="lines",
                        line=dict(width=0),
                        fill="tonexty",
        ...
        if final_freqs:
            hist_data = [
                {"Frecuencia": v, "Nivel": l.capitalize()}
                for l in ["pares", "tercias", "cuartetos"]
                if final_freqs.get(l)
                for v in final_freqs[l].values()
            ]
            if hist_data:
        ...
        if not df_historico.empty and "afinidad_pares" in df_historico.columns:
            affinity_data = [
                {"Afinidad": v, "Nivel": l.capitalize()}
                for l in ["pares", "tercias", "cuartetos"]
                for v in df_historico[f"afinidad_{l}"]
            ]
